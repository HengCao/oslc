...
public class ServletListener implements ServletContextListener {
	private static final String PROPERTY_SCHEME = ServletListener.class
			.getPackage().getName() + ".scheme";
	private static final String PROPERTY_PORT = ServletListener.class
			.getPackage().getName() + ".port";
	private static final int REGISTRATION_DELAY = 5000; // Delay before
														// contacting
														// OSLC4JRegistry
	...
	public void contextDestroyed(final ServletContextEvent servletContextEvent) {
		...
		if (client != null) {
			...
			if (!client.getClient().getUri().contains(HOST)) {
				try {
					client.deregisterServiceProvider(ServiceProviderSingleton
							.getServiceProviderURI());
				} catch (final Exception exception) {
					logger.log(
							Level.SEVERE,
							"Unable to deregister with service provider catalog",
							exception);
				} finally {
					client = null;
	...
	private class RegistrationTask extends TimerTask {
		...
		public void run() {
			...
			try {
				...
				client = new ServiceProviderRegistryClient(
						JenaProvidersRegistry.getProviders());

				serviceProviderURI = client
						.registerServiceProvider(serviceProvider);

				ServiceProviderSingleton
						.setServiceProviderURI(serviceProviderURI);
				...
				logger.log(Level.SEVERE,
						"Unable to register with service provider catalog",
						exception);
			...
			try {
				final Populate populate = new Populate(basePath,
					...
					Persistence.load(basePath,serviceProviderURI) ;
			//		populate.populate();
					Persistence.retrive(basePath);
					System.out.println("2"+Persistence.getResources().length);
